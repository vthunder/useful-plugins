...
export async function executeThingsURL(command, params) {
    const queryParams = [];
    ...
    if (url.length > MAX_URL_LENGTH) {
        throw new ThingsError('URL too long. Consider using fewer items or shorter text.', 'URL_TOO_LONG', { length: url.length, max: MAX_URL_LENGTH });
    }
    // Execute
    try {
        ...
        throw new ThingsError(`Failed to execute Things URL: ${error}`, 'URL_EXECUTION_FAILED', { command, error: String(error) });
    }
}
export async function executeThingsJSON(jsonData) {
    ...
    if (url.length > MAX_URL_LENGTH * 4) { // JSON URLs can be longer
        throw new ThingsError('JSON data too large for URL. Consider creating fewer items at once.', 'JSON_TOO_LARGE', { length: url.length });
    }
    try {
        await execAsync(`open "${url}"`, { timeout: 10000 });
    }
    catch (error) {
        throw new ThingsError(`Failed to execute Things JSON: ${error}`, 'JSON_EXECUTION_FAILED', { error: String(error) });
    }
}
//# sourceMappingURL=urlscheme.js.map