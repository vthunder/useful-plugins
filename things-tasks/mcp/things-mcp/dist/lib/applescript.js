 ...
 */
export async function executeAppleScriptFile(scriptName, args = [], options = {}) {
    ...
    const safeArgs = args.map((arg, index) => validateAppleScriptArg(arg, `argument[${index}]`));
    ...
    try {
        ...
        throw new ThingsScriptError(scriptName, errorObj.stderr || errorObj.message || 'Unknown error');
    }
}
/**
 * Test if Things 3 is installed and accessible
 */
export async function testThingsAvailable() {
    try {
        const { stdout } = await execAsync('osascript -e \'tell application "System Events" to name of application processes\'', { timeout: 5000 });
